...
def main():
    first_dataset_path = Path('./FirstDataset/valid')
    second_dataset_path = Path('./SecondDataset/valid')
    ...
    for scan_path in first_dataset_path.iterdir():
        if not scan_path.name.endswith('mask.nii.gz'):
            ...
            # Create data folder
            folder_path = './valid/'+str(scan_path)[19:-7]
    ...
    for scan_path in second_dataset_path.iterdir():
        ...
        folder_path = './valid/'+str(scan_path)[20:]
        os.mkdir(folder_path)
